...
#note that for the one drum scenario it is hard to reduce the power to 40% without violating the speed limit
...
#---
#loop to update the reference power history and match the time and power vectors 
#---
ref_old = pow_point[0]
for it in range(nt):
    if it > 0:
...
#---
#Don't change (max reactivity worth for all 8 drums, about 26 pcm/deg of movement)
...
# Adjust gains based on number of drums (from reactivity per deg) --> maximum is 26.11e-5
...
#PID controller
def pid_controller(t, measurement, setpoint, Kp, Ki, Kd, Kaw, T_C, max_val, min_val, max_rate, init, u0):
...
#--initiliaze the controller (this is an arbitrary call to get init=True and u0=u0 passed)
pid_controller(0, 0, 0, Kp, Ki, Kd, Kaw, T_C, max_val, min_val, max_rate, init=True, u0=u0);

#--Now start the simualtion loop
for i in range(nt - 1):
    dx = reactor_dae(x[i, :], u[i], Rho_d0, Reactivity_per_degree)
    x[i + 1, :] = x[i, :] + dx * dt
    u[i + 1] = pid_controller(time[i], x[i + 1, 0], ref[i + 1], 
                              Kp, Ki, Kd, Kaw, T_C, 
                              max_val, min_val, max_rate, False, 0)
...
print(f'Min and Max drum position at any point in time: Max: {max_position:.1f}, Min: {min_position:.1f}')
print(f'Min and Max drum speed at any point in time: Max: {max_speed:.2f} (allowed: {max_rate_orig}), Min: {min_speed:.2f} (allowed: {-max_rate_orig})')
print('************************************')
